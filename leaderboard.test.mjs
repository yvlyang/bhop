...
import map from './maps/kz_hub.json' with { type: 'json' };
...
assert.equal(boardKey('kz_hub', 'scroll'), 'kz_hub_clean:scroll');
assert.equal(boardKey('kz_hub_clean', 'scroll'), boardKey('kz_hub', 'scroll'));
assert.equal(boardKey('kz_hub', 'auto'), null);
...
let response = await get(leaderboard, 'map=kz_hub&mode=scroll');
...
assert.equal((await get(leaderboard, 'map=kz_hub&mode=auto')).status, 400);
...
await post(finish, { token: tokenA, map: 'kz_hub', mode: 'scroll', time: 40 });
assert.equal((await (await get(leaderboard, 'map=kz_hub&mode=scroll')).json()).totalRuns, 1);
...
const run = { map: 'kz_hub', mode: 'scroll', time, replay: { v: 1, map: 'kz_hub', mode: 'scroll', time, n: 601, startIdx: 1, startTime: 0, events: [], b64: bytes.toString('base64') } };
assert.equal(validateRun(run).ok, true);
assert.equal(validateRun({ ...run, replay: { ...run.replay, map: 'kz_hub_clean' } }).ok, true);
...
assert.equal((await get(replay, `map=kz_hub&mode=scroll&id=${saved.id}`)).status, 200);
