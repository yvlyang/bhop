import { MODES, ensureSchema, json, noDatabase, playerIdFromToken } from '../../lib/leaderboard.js';
...
export async function onRequestPost({ request, env }) {
  ...
  const now = Date.now();
  const row = await db.prepare('SELECT runs, updated_at FROM run_counts WHERE player_id = ?1 AND mode = ?2').bind(playerId, body.mode).first();
