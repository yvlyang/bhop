...
import kzMap from '../maps/kz_hub_clean.json' with { type: 'json' };

export const MODES = ['auto', 'scroll'];
export function boardKey(map = 'bhop_brick', mode) {
  if (!MODES.includes(mode)) return null;
  if (map === 'bhop_brick') return mode;
  return map === 'kz_hub_clean' && mode === 'scroll' ? 'kz_hub_clean:scroll' : null;
}
...
export function validateRun(body) {
  ...
  const map = body?.map ?? 'bhop_brick', kz = map === 'kz_hub_clean';
  if (!boardKey(map, mode)) return fail('unknown map or mode');
  if (r?.map && r.map !== map) return fail('replay map does not match');
