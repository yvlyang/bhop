...
const SCHEMA = [
  ...
  ...['INSERT', 'UPDATE OF name, player_id'].map((event, i) =>
    `CREATE TRIGGER IF NOT EXISTS reserve_run_name_${i} BEFORE ${event} ON runs BEGIN
       INSERT OR IGNORE INTO player_names (name_key, player_id) VALUES (lower(trim(NEW.name)), NEW.player_id);
