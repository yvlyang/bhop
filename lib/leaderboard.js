...
function decodeFrames(b64, n, origin = [0, 0, 0]) {
  ...
  return (i) => {
    const o = i * 14;
    return { x: dv.getInt16(o) / 8 + origin[0], y: dv.getInt16(o + 2) / 8 + origin[1], z: dv.getInt16(o + 4) / 8 + origin[2], eye: dv.getUint8(o + 10) / 3, speed: dv.getUint16(o + 11) / 4, keys: dv.getUint8(o + 13) };
  };
}
...
export function validateRun(body) {
  ...
  for (let i = r.startIdx; i <= finish; i++) {
    for (let ti = 0; ti < triggers.length; ti++) {
      const t = triggers[ti];
      if (disabled.has(t.name) || !inZone(prev, t.zone)) { touching.delete(ti); continue; }
      if (touching.has(ti)) continue;
      touching.add(ti);
      let returned = false;
      for (const [action, arg] of t.actions) {
        ...
        if (action === 'fall' || action === 'reset') { returned = true; break; }
      }
      if (returned) break;
    }
    const f = frame(i);
    const duckShift = kz && Math.abs(f.eye - prev.eye) >= 8;
    ...
    if (kz) {
      ...
      const dy = f.y - dest.pos[2];
      const verticalError = duckShift ? Math.min(Math.abs(dy), Math.abs(dy - 18), Math.abs(dy + 18)) : Math.abs(dy);
      const near = Math.hypot(f.x - dest.pos[0], f.z + dest.pos[1]) < 3 && verticalError < 6;
      validTeleport = near;
    }
    const verticalLimit = duckShift ? MAX_VELOCITY * DT + 18 + 2 + 0.25 : 45;
    if ((horizontal > allowed || Math.abs(f.y - prev.y) > verticalLimit) && !validTeleport) return fail(`impossible movement at tick ${i}`);
    prev = f;
  }
